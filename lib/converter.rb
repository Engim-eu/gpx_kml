...
module CONVERTER
  ...
  class Converter
    def self.gpx_to_kml(gpx_path, output_path)
      ...
      f = File.open("#{output_path}/#{Time.now.strftime('%Y%m%d%H%M%S')}_#{gpx.file_name[0..-5]}.kml", 'w')
      f.write(kml.to_xml)
      f.close
    ...
    def self.kml_to_gpx(kml_path, output_path)
      ...
      f = File.open("#{output_path}/#{Time.now.strftime('%Y%m%d%H%M%S')}_#{kml.file_name[0..-5]}.gpx", 'w')
      f.write(gpx.to_xml)
      f.close
      puts 'conversion completed'
    end
